...
├── hook.py             # 备用 hook 脚本（HTTP hook 不可用时使用）
...
### 备用方案：command hook（HTTP hook 不可用时）

```json
{
  "hooks": {
    "PreToolUse": [
      { "hooks": [{ "type": "command", "command": "python /你的路径/CCmonitor/hook.py" }] }
    ],
    "PostToolUse": [
      { "hooks": [{ "type": "command", "command": "python /你的路径/CCmonitor/hook.py" }] }
    ],
...
| `/api/event` | POST | 通用事件端点（hook.py 调用） |
...
|------|--------|------|
| `PORT` | `9090` | 服务器端口 |
| `CCMONITOR_URL` | `http://localhost:9090/api/event` | hook.py 发送事件的地址 |
